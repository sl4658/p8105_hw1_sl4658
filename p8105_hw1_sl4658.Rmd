...
When we tried to convert all variables to numeric, character variables were not able to convert to numeric. Consequently, NAs were introduced by coercion. This explained why we were not able to get the mean of character variables in the previous code (the variables turn into NAs).
...
sex with 2 levels, which are `r unique(penguins$sex)`;
year, which has mean of `r mean(na.omit(penguins$year))` and SD of `r sd(na.omit(penguins$body_year))`.
